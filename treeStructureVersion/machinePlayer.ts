...
export class MachinePlayer implements Player {
    ...
    public moveWithOpponent(
        ...
        for (let i = 0; i < tempChildrenList.length; i++) {
            if (
                tempChildrenList[i].name[0] === opponentMovePos[0] &&
    ...
    public expand(): void {
        ...
        this._allChoices.forEach((each) => {
            this._temp.appendChild(
                new Node(this._temp, each, [null, Infinity])
            );
        });
    ...
    public backPropagate(state: string): void {
        ...
        let probe: Node =
            state === "2" ? this._temp.childrenList[0] : this._temp;
        let depth: number = 0;
    ...
    public minimax(aTree: Node, isMaximizer: boolean): void {
        let needRecursion: [boolean, number[]] = this.hasNullValue(
            aTree.childrenList
        );
        if (needRecursion[0]) {
        ...
        function ascendingSort(
            a: ["ROOT" | [number, number], any],
            b: ["ROOT" | [number, number], any]
        ): number {
            if (a[1] === b[1]) return 0;
